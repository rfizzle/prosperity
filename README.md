...
- **Prospector's Compass** — A gold-cased compass whose needle points at the nearest container *you* haven't looted — per player, blacklist- and refresh-aware. Turns up rarely in Frontier+ chest loot, or craft one deliberately: a vanilla compass framed in gold with an end rod for the needle and a netherite ingot at the crown. Loot the target and it swings to the next candidate; with nothing unlooted in range it spins like a vanilla compass in the Nether.
...
- **Fishing loot scaling** — The catch scales with the distance tier at your bobber: the tier's quality biases the roll toward the treasure category, and stackable catches get the tier's multiplier (non-stackables like enchanted books are never multiplied). Luck of the Sea stacks on top. Independently togglable.
...
- **Loot Modifier API** — A Fabric-style event (`LootModifierCallback.EVENT`) lets other mods inject luck, stack multipliers, and custom data into the loot context after distance scaling but before the table resolves. Vanilla `generic.luck` is wired in by default.
...
- **Milestone advancements** — A Prosperity advancement tab turns exploration and looting into concrete goals: the first loot in each distance tier, lifetime volume (10 / 50 / 250 containers), and structure variety (3 / 8 / 15 distinct types), all hung off a Prospector's Compass root. Granted server-side on real instanced generations only, and carried across relogs on your player data.
...
- **HUD tier badge** — A compact, shared-suite HUD element (priority 3 in the rfizzle HUD strip) shows your current distance loot tier (e.g. `[chest] Frontier`). Hold the **Peek Loot Detail** bind (Left Alt by default, rebindable under Controls → Prosperity) to expand it into a full-screen detail panel covering your exact distance and tier progress, the whole tier ladder, and the unlooted containers around you. Independently togglable.
...
- **Party loot mode** — Optional co-op sharing: players on the same scoreboard team share one loot instance per container — the first to open it rolls the pot, teammates see the same inventory, and it refreshes on one shared cooldown. Teamless players keep their own instances, a container already open by a teammate is refused rather than desynced, and leaving a team never re-rolls a chest you opened with it. A party/social mod can drive the grouping through the API. Off by default.
- **Commands** — `/prosperity info` reports your current tier and multipliers and `/prosperity stats` your lifetime loot stats (both open to any player); op-level `reset`, `refresh`, and `reload` manage instanced data and config.
